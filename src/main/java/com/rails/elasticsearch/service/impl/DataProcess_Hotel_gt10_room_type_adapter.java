	...
	public boolean mqData2Elasticsearch(List<MessageRequest> datas) {
		...
		for (int i = 0; i < datas.size(); i++) {
			...
			if (parseObject != null) {
				...
				String roomId = parseObject.getString("roomTypeAdapterId") + "_" + parseObject.getString("city");
		...
		logger.info("DataProcess_Hotel_gt10_room_type_adapter批量导入后剩余的数据导入的总条数为：" + (count - bulkNum * loopNum) + ",状态为："
				+ response.status());
		return false;
	}
}
