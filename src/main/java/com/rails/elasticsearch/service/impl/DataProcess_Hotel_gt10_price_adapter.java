...
public class DataProcess_Hotel_gt10_price_adapter implements MQ2Elasticsearch {
	...
	public boolean mqData2Elasticsearch(List<MessageRequest> datas) {
		...
		for (int i = 0; i < datas.size(); i++) {
			...
			if (parseObject != null) {
				// 格式化时间类型
				parseObject = JSONUtils.formatDate("yyyy-MM-dd HH:mm:ss", parseObject, "starCancel");
				parseObject = JSONUtils.formatDate("yyyy-MM-dd HH:mm:ss", parseObject, "endCancel");
				...
				String priceId = parseObject.getString("productPriceId") + "_" + parseObject.getString("city");

				IndexRequestBuilder setSource = client.prepareIndex("price", "price", priceId).setRouting(priceId)
